...
function ResourceTable({ title, endpointPath, collectionKey }) {
  const [items, setItems] = useState([])
  const [pagination, setPagination] = useState({ page: null, pageSize: null, total: 0 })
  const [status, setStatus] = useState('loading')
  const [errorMessage, setErrorMessage] = useState('')

  const endpointUrl = useMemo(() => `${getApiOrigin()}/api/${endpointPath}/`, [endpointPath])
  ...
  const usingCodespaceEndpoint = Boolean(getCodespaceName())
